...
def test(dataL, la="NONE"):
    ...
    with torch.no_grad():
        with tqdm(total=len(dataL)) as tepoch:
            for (dAB, dBA, c, gAB_Origin, y) in dataL:
                _, pred1 = fwd_pass((dAB, c, gAB_Origin))
                _, pred2 = fwd_pass((dBA, c, gAB_Origin))
    ...
    for epoch in range(P["EPOCHES"]):
        ...
        for i, (train_ind, valid_ind) in enumerate(KF.split(X)):
            ...
            with tqdm(total=len(trainDL)) as tepoch:
                for j, (dAB, dBA, c, gAB_Origin, y) in enumerate(trainDL):
                    loss1, _ = fwd_pass((dAB, c, gAB_Origin), y, train=True)
                    loss2, _ = fwd_pass((dBA, c, gAB_Origin), y, train=True)
...
def main(study):
    ...
    with open(f"./data/{study}_train_addproteintoDruggraphyWithProteinInterconnection.pkl", 'rb') as fp:
        ds = pickle.load(fp)
    X = [i[0] for i in ds]
    Y = [i[1] for i in ds]
